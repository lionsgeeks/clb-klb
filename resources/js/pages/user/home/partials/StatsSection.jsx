...
export default function StatsSection() {
    return (
        <section className="relative border-b border-border bg-cl-black py-12 lg:py-16">
            ...
            <div className="relative container grid grid-cols-2 gap-8 lg:grid-cols-4 lg:gap-12">
                {stats.map((stat, i) => (
                    <div key={i} className="text-center">
                        <p className="text-3xl font-extrabold tracking-tight text-alpha sm:text-4xl lg:text-5xl">
                            {stat.value}
                        </p>
                        <p className="mt-1 text-sm font-medium tracking-wider text-cl-white/80 uppercase">
                            <TransText
                                fr={stat.label.fr}
                                ar={stat.label.ar}
                                nl={stat.label.nl}
